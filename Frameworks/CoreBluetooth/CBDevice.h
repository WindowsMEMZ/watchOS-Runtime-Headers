...
@interface CBDevice : NSObject<CUXPCCodable, NSSecureCoding, NSCopying, CBDictionaryCodable> {
  /* instance variables */
  struct { unsigned char x[3] bitArray; } _changedTypesInternal;
...
- (BOOL)changedTypesContainCBDiscovery:(id)cbdiscovery;
- (BOOL)changedTypesContainTypes:(const struct { unsigned char x0[3] } *)types;
...
- (BOOL)decryptNearbyInfoV2PayloadPtr:(const char *)ptr payloadLength:(unsigned long long)length key:(const char *)key keyLength:(unsigned long long)length decryptedPtr:(void *)ptr;
- (BOOL)discoveryTypesContainCBDiscovery:(id)cbdiscovery;
- (BOOL)discoveryTypesContainType:(int)type;
- (BOOL)discoveryTypesContainTypes:(const struct { unsigned char x0[3] } *)types;
- (BOOL)isEquivalentToCBDevice:(id)cbdevice compareFlags:(unsigned int)flags;
...
- (void)updateWithCBIdentity:(id)cbidentity;
...
- (void)_parseStatusOne:(unsigned char)one deviceFlags:(unsigned long long *)flags primaryPlacement:(int *)placement secondaryPlacement:(int *)placement;
