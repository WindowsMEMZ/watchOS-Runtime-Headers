//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.454, BuildTime: 2024-02-08T22:07:34Z)
//
//    - LC_BUILD_VERSION:  Platform: watchOSSimulator, MinOS: 10.2, SDK: 10.2, Tool: ld (902.8)
//    - LC_SOURCE_VERSION: 50.0.6.0.0
//
#ifndef SKPlaySound_h
#define SKPlaySound_h
@import Foundation;

#include "SKAction.h"
#include "SKSoundSource.h"

@class NSString;

@interface SKPlaySound : SKAction {
  /* instance variables */
  NSString *_filePath;
  NSString *_fileName;
  struct SKCPlaySound { undefined * * x0; unsigned int x1; float x2; id /* block */ x3; id x4; BOOL x5; double x6; double x7; float x8; float x9; double x10; BOOL x11; BOOL x12; id /* block */ x13; long long x14; float x15; float x16; float x17; float x18; id x19; BOOL x20; BOOL x21; } * _mycaction;
  struct CGPoint { double x; double y; } _position;
  SKSoundSource *_soundSource;
}

/* class methods */
+ (BOOL)supportsSecureCoding;
+ (id)playSoundFileNamed:(id)named atPosition:(struct CGPoint { double x0; double x1; })position waitForCompletion:(BOOL)completion;
+ (id)_audioURLWithName:(id)name bundle:(id)bundle;

/* instance methods */
- (id)init;
- (id)initWithCoder:(id)coder;
- (void)encodeWithCoder:(id)coder;
- (id)copyWithZone:(struct _NSZone *)zone;
- (id)reversedAction;
@end

#endif /* SKPlaySound_h */
